...
void SourceFile::optimizeIR(){
    IROptimizer irOptimize(this,targetMachine.get());
    irOptimize.prepare();
    irOptimize.optimize();
    std::cout << "Optimized IR Code: " << irOptimize.getIRString() <<std::endl;
}
void SourceFile::generateIR(){
    IRGenerator irGenerate(this);
    assert(ast!=nullptr);
    irGenerate.visit(ast);
    std::cout << "Unoptimized IR code: " << irGenerate.getIRString() << std::endl;
}
